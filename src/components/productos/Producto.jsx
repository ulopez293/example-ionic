...
function Producto(props) {
  ...
  let [renderProducto, setRenderProducto] = useState(props.producto)
  ...
  useEffect(() => {
    if(estado.productos.length >= 1){
      for (let element of estado.productos) {
        if(element.identificador==props.producto.identificador){
          setCantidadProducto(element.cantidad)
        }
      }
    }
    return () => {
      console.log("Limpiando Producto..")
    }
  }, [estado.productos])
  ...
  const agregarCarrito = () => {
    ...
    let tempProducto = props.producto
    if(tempProducto.cantidad==undefined){
      tempProducto.cantidad = cantidadProducto
    }
    tempProducto.cantidad++
    if(estado.productos.length >=1 ) {
      let temporalEstado = estado.productos
      temporalEstado.push(tempProducto)
      let hash = {};
      temporalEstado = temporalEstado.filter(o => hash[o.identificador] ? false : hash[o.identificador] = true);
      for (let i=0; i < temporalEstado.length; i++) {
        if (temporalEstado[i].identificador==props.producto.identificador){
          temporalEstado[i].cantidad = tempProducto.cantidad
        }
      }
      estadoAction({
  ...
  const eliminarRegistro = () => {
    ...
    request.onerror = function(event) { alert("Error al cargar Base de Datos.") }
    request.onsuccess = function(event) {
      db = event.target.result
      let transaction = db.transaction(["productos"], "readwrite")
                          .objectStore("productos")
                          .delete(parseInt(props.producto.identificador))
  ...
  const editarRegistro = ()=> {
    ...
    if(!letrasRegex.test(nombreProducto)) return

    let descripcionProducto = prompt("Ingrese la Descripcion del Producto: ")
    if(!letrasRegex.test(descripcionProducto)) return

    let precioProducto = prompt("Ingrese el Precio del Producto: " )
    if(!numerosRegex.test(precioProducto)) return
    ...
    request.onsuccess = function(event) {
      ...
      let transaction = objectStore.get(props.producto.identificador)
      transaction.onsuccess = function(event) {
